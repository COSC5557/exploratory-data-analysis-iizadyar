...
# RandomForest with preprocessing as part of a pipeline
clfr_pipeline = Pipeline([
    ('preprocessor', preprocessor),
    ('classifier', clfr_no_preprocessing)
])

# Cross-validation score without preprocessing
...
# Visualization of feature distributions before and after preprocessing (unchanged)
# This block of code remains the same

# Cross-validation score with preprocessing
...
# Visualization of feature distributions before and after preprocessing
...
for i, feature in enumerate(features):
    ...
    axs[1, i].hist(X_red[feature].apply(np.log1p), bins=20, color='orange', alpha=0.5, label='After')  # Using np.log1p directly
